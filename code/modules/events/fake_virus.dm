	...
	if(defacto_min)
		for(var/i=1; i<=rand(1,defacto_min); i++)
			...
			if(prob(25))//1/4 odds to get a spooky message instead of coughing out loud
				...
				addtimer(CALLBACK(onecoughman, TYPE_PROC_REF(.mob, emote), pick("кашляет", "чихает", "сопит")), rand(30,150))//deliver the message with a slightly randomized time interval so there arent multiple people coughing at the exact same time
